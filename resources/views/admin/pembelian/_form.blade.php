...
@if(isset($pembelian) && $pembelian->detailPembelian->count() > 0)
  @foreach($pembelian->detailPembelian as $index => $detail)
    <div class="detail_row grid grid-cols-4 gap-3 items-end mb-3">
      <div>
      ...
      <div>
        ...
        <input name="details[{{ $index }}][harga_beli]" type="number" step="0.01" min="0"
      ...
      <div>
        ...
        <input name="details[{{ $index }}][kuantitas]" type="number" min="1"
               value="{{ $detail->kuantitas }}"
               class="w-full rounded-md border px-3 py-2 text-sm border-gray-200 focus:border-blue-500">
        <p class="text-xs text-rose-600 mt-1 hidden" data-error="kuantitas_{{ $index }}"></p>
      </div>
      <div class="flex items-center gap-2">
        <button type="button" class="add_detail inline-flex items-center px-3 py-2 bg-green-600 text-white text-sm rounded-md hover:bg-green-700">
          Tambah
        </button>
        @if($index > 0)
          <button type="button" class="remove_detail inline-flex items-center px-3 py-2 bg-red-600 text-white text-sm rounded-md hover:bg-red-700">
            Hapus
          </button>
        @else
          <button type="button" class="remove_detail hidden">
            Hapus
          </button>
        @endif
      </div>
