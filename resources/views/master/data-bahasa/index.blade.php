...
<div class="container mx-auto">
  <div class="flex flex-col items-start mb-4 sm:mb-6">
    <h1 class="text-2xl sm:text-2xl font-semibold text-gray-800 text-left">Data Bahasa</h1>
    <p class="text-xs sm:text-sm text-gray-500 mt-1 text-left">Ringkasan data bahasa koperasi</p>
  </div>

  <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-bahasa.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-bahasa.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <a href="{{ route('master.data-bahasa.create') }}" 
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="w-24 sm:w-32 px-2 sm:px-4 py-2 sm:py-3 border-r border-gray-200">ID Bahasa</th>
          <th class="flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r">Nama Bahasa</th>
          <th class="w-32 sm:w-40 px-2 sm:px-4 py-2 sm:py-3 border-r">Aksi</th>
        </tr>
      </thead>
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($bahasas ?? collect() as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r border-gray-100">{{ $item->id_bahasa }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->nama_bahasa }}</td>
            <td class="px-2 sm:px-4 py-2 text-center border-r">
              <div class="flex justify-center items-center gap-2 sm:gap-3">
                <a href="{{ route('master.data-bahasa.edit', $item->id_bahasa) }}"
                ...
                <form action="{{ route('master.data-bahasa.destroy', $item->id_bahasa) }}" method="POST"
                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus data bahasa ini?');">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
  ...
  <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
    <div class="text-xs sm:text-sm text-gray-600">
      @if(isset($bahasas) && $bahasas->total())
        Menampilkan {{ $bahasas->firstItem() }} sampai {{ $bahasas->lastItem() }} dari {{ $bahasas->total() }} data
      @endif
    </div>
    <div>
      @if(isset($bahasas) && method_exists($bahasas, 'links'))
        {{ $bahasas->appends(request()->query())->links('vendor.pagination.custom') }}
      @endif
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-bahasa.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
