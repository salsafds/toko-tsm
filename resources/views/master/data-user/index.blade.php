...
<div class="container mx-auto">
    <div class="flex flex-col items-start mb-4 sm:mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 text-left">Data User</h1>
        <p class="text-sm text-gray-500 mt-1 text-left">Daftar pengguna sistem</p>
    </div>
    ...
    <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <form method="GET" action="{{ route('master.data-user.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
            ...
            <a href="{{ route('master.data-user.create') }}" 
            ...
            <table class="min-w-full divide-y divide-gray-200 table-auto">
                <thead class="bg-gray-50">
                    <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <th class="min-w-[120px] px-3 py-3 border-r whitespace-nowrap">Username</th>
                        <th class="min-w-[100px] px-3 py-3 border-r whitespace-nowrap">Role</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($users as $item)
                        <tr class="hover:bg-gray-50 transition-colors text-xs sm:text-sm">
                            <td class="px-3 py-2 border-r border-gray-100 whitespace-nowrap">{{ $item->id_user }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->nama_lengkap }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->username }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->role->nama_role ?? '-' }}</td>
                            <td class="px-3 py-2 border-r text-center">
                                <div class="flex justify-center items-center gap-1">
                                    <a href="{{ route('master.data-user.edit', $item->id_user) }}"
                                       class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 hover:bg-blue-200">
                                        Edit
                                    </a>
                                    <form action="{{ route('master.data-user.destroy', $item->id_user) }}" method="POST"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchData() {
        ...
        fetch(`{{ route('master.data-user.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
