...
<div class="container mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-gray-500 mt-1">Daftar barang dan informasi stok</p>
    </div>
  </div>
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('master.data-barang.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-700 text-white rounded-md text-sm hover:bg-blue-800">
        + Tambah
      </a>
    </div>
  </div>
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="px-4 py-3 border-r">ID Barang</th>
          <th class="px-4 py-3 border-r">Nama Barang</th>
          <th class="px-4 py-3 border-r">Kategori</th>
          <th class="px-4 py-3 border-r">Stok</th>
          <th class="px-4 py-3 border-r">Satuan</th>
          <th class="px-4 py-3 border-r">Harga Retail</th>
          <th class="px-4 py-3 border-r">Harga Beli</th>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($barang as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->id_barang }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->nama_barang }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->kategoriBarang->nama_kategori }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->stok }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->satuan->nama_satuan }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ number_format($item->retail, 0, ',', '.') }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ number_format($item->harga_beli, 0, ',', '.') }}</td>
