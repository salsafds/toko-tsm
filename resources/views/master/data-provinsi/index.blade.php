...
<div class="container mx-auto">
  <div class="flex flex-col items-start mb-4 sm:mb-6">
    <h1 class="text-2xl sm:text-2xl font-semibold text-gray-800 text-left">Data Provinsi</h1>
    <p class="text-xs sm:text-sm text-gray-500 mt-1 text-left">Ringkasan data provinsi koperasi</p>
  </div>

  <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-provinsi.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-provinsi.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <a href="{{ route('master.data-provinsi.create') }}" 
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="w-24 sm:w-32 px-2 sm:px-4 py-2 sm:py-3 border-r border-gray-200">ID Provinsi</th>
          <th class="flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r">Nama Provinsi</th>
          <th class="w-32 sm:w-40 px-2 sm:px-4 py-2 sm:py-3 border-r">Aksi</th>
        </tr>
      </thead>
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($provinsis ?? collect() as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r border-gray-100">{{ $item->id_provinsi }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->nama_provinsi }}</td>
            <td class="px-2 sm:px-4 py-2 text-center border-r">
              <div class="flex justify-center items-center gap-2 sm:gap-3">
                <a href="{{ route('master.data-provinsi.edit', $item->id_provinsi) }}"
                ...
                <form action="{{ route('master.data-provinsi.destroy', $item->id_provinsi) }}" method="POST"
                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus data provinsi ini?');">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
  ...
  <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
    <div class="text-xs sm:text-sm text-gray-600">
      @if(isset($provinsis) && $provinsis->total())
        Menampilkan {{ $provinsis->firstItem() }} sampai {{ $provinsis->lastItem() }} dari {{ $provinsis->total() }} data
      @endif
    </div>
    <div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-provinsi.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
