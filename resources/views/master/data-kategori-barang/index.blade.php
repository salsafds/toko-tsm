...
<div class="container mx-auto">
  <div class="flex flex-col items-start mb-4 sm:mb-6">
    <h1 class="text-2xl sm:text-2xl font-semibold text-gray-800 text-left">Data Kategori Barang</h1>
    <p class="text-xs sm:text-sm text-gray-500 mt-1 text-left">Ringkasan data kategori barang koperasi</p>
  </div>
  ...
  <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-kategori-barang.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <a href="{{ route('master.data-kategori-barang.create') }}" 
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="w-24 sm:w-32 px-2 sm:px-4 py-2 sm:py-3 border-r border-gray-200">ID Kategori</th>
          <th class="flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r">Nama Kategori</th>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($kategoris ?? collect() as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r border-gray-100">{{ $item->id_kategori_barang }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->nama_kategori }}</td>
