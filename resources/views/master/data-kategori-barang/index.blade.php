...
<div class="container mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-gray-800">Data Kategori Barang</h1>
      <p class="text-sm text-gray-500 mt-1">Ringkasan data kategori barang koperasi</p>
    </div>
  </div>
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-3">
      <form method="GET" action="{{ route('master.data-kategori-barang.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('master.data-kategori-barang.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-700 text-white rounded-md text-sm hover:bg-blue-800">
        + Tambah
      </a>
    </div>
  </div>
...
<thead class="bg-gray-50">
  <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
    <th class="w-32 px-4 py-3 border-r border-gray-200">ID Kategori</th> 
    <th class="w-auto flex-1 border-r px-4 py-3">Nama Kategori</th>
    <th class="w-40 px-4 py-3 border-r">Aksi</th> <!-- Increased width to w-40 -->
  </tr>
    </thead>
    <tbody class="bg-white divide-y divide-gray-100">
      @forelse($kategoris ?? collect() as $item)
        <tr class="hover:bg-gray-50 transition-colors">
          <td class="w-auto flex-1 border-r px-4 py-2 text-sm text-gray-700 border-gray-100">{{ $item->id_kategori_barang }}</td>
          <td class="w-auto flex-1 border-r px-4 py-2 text-sm text-gray-700"> {{ $item->nama_kategori }}</td> 
          <td class="w-40 px-4 py-2 border-r text-center"> <!-- Increased width to w-40 -->
            <div class="flex justify-center items-center gap-3"> <!-- Changed gap-2 to gap-3 to match Tambah container -->
              <!-- Tombol Edit -->
              <a href="{{ route('master.data-kategori-barang.edit', $item->id_kategori_barang) }}"
              ...
              <!-- Tombol Hapus -->
              <form action="{{ route('master.data-kategori-barang.destroy', $item->id_kategori_barang) }}" method="POST"
                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus data kategori ini?');">
                @csrf
                @method('DELETE')
                <button type="submit"
  ...
  <div class="mt-4 flex items-center justify-between">
    <div class="text-sm text-gray-600">
      @if(isset($kategoris) && $kategoris->total())
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-kategori-barang.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
