...
<div class="container mx-auto">
  <div class="flex flex-col items-start mb-4 sm:mb-6">
    <h1 class="text-2xl sm:text-2xl font-semibold text-gray-800 text-left">Data Satuan</h1>
    <p class="text-xs sm:text-sm text-gray-500 mt-1 text-left">Ringkasan data satuan koperasi</p>
  </div>

  <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-satuan.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-satuan.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <a href="{{ route('master.data-satuan.create') }}" 
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="w-24 sm:w-32 px-2 sm:px-4 py-2 sm:py-3 border-r border-gray-200">ID Satuan</th>
          <th class="flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r">Nama Satuan</th>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($satuans ?? collect() as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r border-gray-100">{{ $item->id_satuan }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->nama_satuan }}</td>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-satuan.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
