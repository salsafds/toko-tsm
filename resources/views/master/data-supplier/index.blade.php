...
<div class="container mx-auto">
  <div class="flex flex-col items-start mb-4 sm:mb-6">
    <h1 class="text-2xl sm:text-2xl font-semibold text-gray-800 text-left">Data Supplier</h1>
    <p class="text-xs sm:text-sm text-gray-500 mt-1 text-left">Daftar supplier dan informasi kontak koperasi</p>
  </div>

  <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-supplier.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      <form method="GET" action="{{ route('master.data-supplier.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto">
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          ...
          <th class="flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r">Nama</th>
          <th class="w-32 sm:w-40 px-2 sm:px-4 py-2 sm:py-3 border-r">Telepon</th>
          <th class="w-32 sm:w-40 px-2 sm:px-4 py-2 sm:py-3 border-r">Email</th>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($suppliers as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r border-gray-100">{{ $item->id_supplier }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->nama_supplier }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->telepon_supplier }}</td>
            <td class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-gray-700 border-r">{{ $item->email_supplier }}</td>
        ...
        @empty
          <tr>
            <td colspan="5" class="px-4 py-6 sm:py-8 text-center text-xs sm:text-sm text-gray-500">
  ...
  <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
    <div class="text-xs sm:text-sm text-gray-600">
      @if(isset($suppliers) && $suppliers->total())
        Menampilkan {{ $suppliers->firstItem() }} sampai {{ $suppliers->lastItem() }} dari {{ $suppliers->total() }} data
      @endif
    </div>
    <div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-supplier.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
