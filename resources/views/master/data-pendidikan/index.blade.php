...
<div class="container mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-gray-800">Data Pendidikan</h1>
      <p class="text-sm text-gray-500 mt-1">Ringkasan aktivitas & statistik koperasi</p>
    </div>
  </div>
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-3">
      <form method="GET" action="{{ route('master.data-pendidikan.index') }}" class="flex items-center gap-2">
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('master.data-pendidikan.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-700 text-white rounded-md text-sm hover:bg-blue-800">
        + Tambah
      </a>
    </div>
  </div>
    ...
    <table class="min-w-full divide-y divide-gray-200 table-auto"> 
      <thead class="bg-gray-50">
        <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
          <th class="w-32 px-4 py-3 border-r border-gray-200">ID Pendidikan</th> 
          <th class="w-auto flex-1 border-r px-4 py-3">Tingkat Pendidikan</th>
          <th class="w-32 px-4 py-3 border-r">Aksi</th>
        </tr>
      </thead>
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($pendidikans ?? collect() as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="w-auto flex-1 border-r px-4 py-2 text-sm text-gray-700 border-gray-100">{{ $item->id_pendidikan }}</td>
            <td class="w-auto flex-1 border-r px-4 py-2 text-sm text-gray-700"> {{ $item->tingkat_pendidikan }}</td> 
            <td class="w-32 px-4 py-2 border-r text-center"> 
              <div class="flex justify-center items-center gap-2">
                <!-- Tombol Edit -->
                <a href="{{ route('master.data-pendidikan.edit', $item->id_pendidikan) }}"
                ...
                <!-- Tombol Hapus -->
                <form action="{{ route('master.data-pendidikan.destroy', $item->id_pendidikan) }}" method="POST"
  ...
  <div class="mt-4 flex items-center justify-between">
    <div class="text-sm text-gray-600">
      {{-- showing X to Y of Z --}}
      @if(isset($pendidikans) && $pendidikans->total())
        Menampilkan {{ $pendidikans->firstItem() }} sampai {{ $pendidikans->lastItem() }} dari {{ $pendidikans->total() }} data
      @endif
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchData() {
    ...
    fetch(`{{ route('master.data-pendidikan.index') }}?q=${encodeURIComponent(query)}&per_page=${perPage}`, {
