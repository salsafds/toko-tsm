...
<div class="container mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-gray-800">Data Agen Ekspedisi</h1>
      <p class="text-sm text-gray-500 mt-1">Daftar agen ekspedisi dan kontak</p>
    </div>
  </div>
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('master.data-agen-ekspedisi.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-700 text-white rounded-md text-sm hover:bg-blue-800">
        + Tambah
      </a>
    </div>
  </div>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @forelse($agens as $item)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->id_ekspedisi }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->nama_ekspedisi }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->telepon_ekspedisi }}</td>
            <td class="px-4 py-2 text-sm text-gray-700 border-r">{{ $item->email_ekspedisi }}</td>
            <td class="px-4 py-2 text-center">
              <div class="flex justify-center items-center gap-2">
                <a href="{{ route('master.data-agen-ekspedisi.edit', $item->id_ekspedisi) }}" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-300">Edit</a>

                <form action="{{ route('master.data-agen-ekspedisi.destroy', $item->id_ekspedisi) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data agen ekspedisi ini?');">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-rose-100 text-rose-700 hover:bg-rose-200 focus:outline-none focus:ring-2 focus:ring-rose-300">Delete</button>
                </form>
              </div>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500">Tidak ada data agen ekspedisi.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
