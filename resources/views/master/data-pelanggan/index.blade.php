...
<div class="container mx-auto">
    <div class="flex flex-col items-start mb-4 sm:mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 text-left">Data Pelanggan</h1>
    ...
    <div class="flex flex-col items-start mb-4 gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <form method="GET" action="{{ route('master.data-pelanggan.index') }}" class="flex items-center gap-2 w-full sm:w-auto">
                <div class="relative border rounded-md w-full sm:w-64">
                    ...
                    <input name="q" value="{{ request()->query('q', '') }}" placeholder="Search…" aria-label="Search"
                           class="pl-10 pr-3 py-2 rounded-md border-gray-200 text-sm w-full" />
                </div>
            </form>
            ...
            <table class="min-w-full divide-y divide-gray-200 table-auto">
                <thead class="bg-gray-50">
                    <tr class="text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <th class="min-w-[100px] px-3 py-3 border-r border-gray-200 whitespace-nowrap">ID Pelanggan</th>
                        <th class="min-w-[150px] px-3 py-3 border-r whitespace-nowrap">Nama Pelanggan</th>
                        <th class="min-w-[100px] px-3 py-3 border-r whitespace-nowrap">Kategori</th>
                        <th class="min-w-[100px] px-3 py-3 border-r whitespace-nowrap">Kota</th>
                        <th class="min-w-[150px] px-3 py-3 border-r whitespace-nowrap">Alamat</th>
                        <th class="min-w-[110px] px-3 py-3 border-r whitespace-nowrap">Telepon</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($pelanggans ?? collect() as $item)
                        <tr class="hover:bg-gray-50 transition-colors text-xs sm:text-sm">
                            <td class="px-3 py-2 border-r border-gray-100 whitespace-nowrap">{{ $item->id_pelanggan }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->nama_pelanggan }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ ucfirst(str_replace('_', ' ', $item->kategori_pelanggan)) }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->kota->nama_kota ?? '-' }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap truncate max-w-[150px]" title="{{ $item->alamat_pelanggan }}">{{ $item->alamat_pelanggan }}</td>
                            <td class="px-3 py-2 border-r whitespace-nowrap">{{ $item->nomor_telepon }}</td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchData() {
        ...
        fetch(`{{ route('master.data-pelanggan.index') }}?q=${encodeURIComponent(query)}&per steadfast_page=${perPage}`, {
